...
def ReadNodeGraph() -> list[
    c.NavigationEntry
]:  # None since this will just update the existing nodes
...
def ReadElevations() -> list[tuple[float, float, float]]:
    ...
    for elevation in file:
        elevations.append(c.Elevation(elevation[0], elevation[2], elevation[1]))
            ...
            [
                c.PrefabNavCurve(
                    ...
                    c.Transform(
                        ...
                        quaternion=curve["start"].get("rotationQuat", [0, 0, 0, 1]),
                    ),
                    c.Transform(
                        curve["end"]["x"],
                        curve["end"]["y"],
                        curve["end"]["z"],
                        curve["end"]["rotation"],
                        quaternion=curve["end"].get("rotationQuat", [0, 0, 0, 1]),
                    ),
                    curve["nextLines"],
                    curve["prevLines"],
            ...
            if poi["type"] == c.NonFacilityPOI.LANDMARK:
                ...
                pois.append(
                    c.GeneralPOI(
                        ...
                        TryReadExcept(
                            poi, "z", 0
                        ),  # don't have z, but will have in the future
                        0,
                        0,
                        poi["icon"],
                        poi["label"],
            ...
            if poi["type"] == "road":
                pois.append(
                    c.RoadPOI(
                        ...
                        TryReadExcept(
                            poi, "z", 0
                        ),  # roads don't have z, but will have in the future
                        0,
                        0,
                        poi["icon"],
                        poi["dlcGuard"],
...
def UpdateState(start_time: float, message: str):
    ...
    total_ram_usage = (
        (psutil.Process(os.getpid()).memory_info().rss - start_ram_usage) / 1024 / 1024
    )
    ...
    print(
        f"[dim]{time_string}[/dim] {message} [dim] used {total_ram_usage:.0f}mb of RAM [/dim]",
        end="\n",
    )
...
def ReadData(state=None) -> c.MapData:
    ...
    UpdateState(start_time, "Linked prefabs to descriptions")
